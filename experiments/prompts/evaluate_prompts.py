...
class PromptEvaluator:
    ...
    def __init__(
        ...
        if eval_data_path is None:
            self.eval_data_path = Path(__file__).parent.parent.parent / "data" / "eval.jsonl"
        else:
    ...
    def evaluate_single(self, example: Dict, strategy) -> EvalResult:
        ...
        return EvalResult(
            ...
            cosine_similarity=self.compute_cosine_similarity(gen_summary, example["expected_summary"]),
            precaution_overlap=self.compute_precaution_overlap(gen_precautions, example["expected_precautions"]),
    ...
    def evaluate_strategy(self, strategy) -> Dict[str, Any]:
        ...
        for i, example in enumerate(self.eval_data):
            ...
            if result.error:
                ...
                print(f"ROUGE: {result.rouge_l:.3f} | Cosine: {result.cosine_similarity:.3f}")
        ...
        metrics = {
            ...
            "avg_precaution_overlap": np.mean([r.precaution_overlap for r in successful]),
        ...
        if self.mlflow:
            try:
                with self.mlflow.start_run(run_name=f"{strategy.value}_{datetime.now().strftime('%Y%m%d_%H%M%S')}"):
                    # Log params
        ...
        # Print summary
        print(f"\n  Summary:")
        print(f"    ROUGE-L:     {metrics['avg_rouge_l']:.4f} (±{metrics['std_rouge_l']:.4f})")
    ...
    def _print_comparison(self, results: Dict):
        ...
        print("COMPARISON")
        print("=" * 80)
        print(f"{'Strategy':<20} {'ROUGE-L':>10} {'Cosine':>10} {'Precaution':>12} {'Latency':>10}")
        print("-" * 80)
        
        for name, data in results.items():
            if "metrics" in data:
                m = data["metrics"]
                print(f"{name:<20} {m['avg_rouge_l']:>10.4f} {m['avg_cosine_similarity']:>10.4f} "
