...
def run_mock_evaluation() -> Dict[str, EvalMetrics]:
    ...
    for name, metrics in mock_results.items():
        ...
        if metrics.rouge_l < CI_THRESHOLDS["min_rouge_l"]:
            passed = False
            issues.append(f"ROUGE-L {metrics.rouge_l:.2f} < {CI_THRESHOLDS['min_rouge_l']}")
        
        if metrics.cosine_similarity < CI_THRESHOLDS["min_cosine_similarity"]:
            passed = False
            issues.append(f"Cosine {metrics.cosine_similarity:.2f} < {CI_THRESHOLDS['min_cosine_similarity']}")
        
        if metrics.precaution_overlap < CI_THRESHOLDS["min_precaution_overlap"]:
            passed = False
            issues.append(f"Overlap {metrics.precaution_overlap:.2f} < {CI_THRESHOLDS['min_precaution_overlap']}")
        
        if metrics.error_rate > CI_THRESHOLDS["max_error_rate"]:
            passed = False
            issues.append(f"Error rate {metrics.error_rate:.2f} > {CI_THRESHOLDS['max_error_rate']}")
    ...
    try:
        ...
        for strategy_name, result in results.items():
            if "metrics" in result:
                m = result["metrics"]
                passed = (
                    m["avg_rouge_l"] >= CI_THRESHOLDS["min_rouge_l"] and
                    m["avg_cosine_similarity"] >= CI_THRESHOLDS["min_cosine_similarity"] and
                    m["avg_precaution_overlap"] >= CI_THRESHOLDS["min_precaution_overlap"] and
                    m["error_rate"] <= CI_THRESHOLDS["max_error_rate"]
                )
...
def print_results(results: Dict[str, EvalMetrics]) -> bool:
    ...
    print("=" * 70)
    print(f"{'Strategy':<20} {'ROUGE-L':>10} {'Cosine':>10} {'Overlap':>10} {'Latency':>10} {'Status':>10}")
    print("-" * 70)
    ...
    for name, metrics in results.items():
        ...
        print(f"{name:<20} {metrics.rouge_l:>10.3f} {metrics.cosine_similarity:>10.3f} "
    ...
    print(f"Thresholds: ROUGE-L≥{CI_THRESHOLDS['min_rouge_l']}, "
...
def main():
    ...
    parser.add_argument("--full", action="store_true", 
                        help="Run full evaluation with LLM API calls")
