...
model_version_gauge = Gauge("current_model_version", "Current model version in use")

# Counter: Errors
error_counter = Counter(
    "api_errors_total", "Total number of API errors", ["error_type"]
)
...
async def get_latest_predictions():
    ...
    try:
        ...
        try:
            ...
            for date in predictions_df["Date_only"].unique():
                ...
                daily_summary[str(date)] = {
                    ...
                    "calculated_aqi_min": float(
                        day_data["Predicted_Calculated_AQI"].min()
                    ),
                    "calculated_aqi_max": float(
                        day_data["Predicted_Calculated_AQI"].max()
                    ),
                    "calculated_aqi_mean": float(
                        day_data["Predicted_Calculated_AQI"].mean()
                    ),
                }
        except Exception as e:
        ...
        prediction_period = {
            "start": predictions_df["DateTime"].iloc[0]
            if len(predictions_df) > 0
            else "",
            "end": predictions_df["DateTime"].iloc[-1]
            if len(predictions_df) > 0
            else "",
        }
...
async def get_predictions_by_date(date: str):
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to fetch predictions for date: {str(e)}"
        )
...
async def get_predictions_summary():
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to fetch predictions summary: {str(e)}"
        )
