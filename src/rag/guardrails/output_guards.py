...
class OutputGuard:
    ...
    def _check_hallucination(
        ...
        if indicator_count >= 2:  # Multiple indicators suggest potential issues
            ...
            if indicator_count >= 3:
                violations.append(OutputViolationType.HALLUCINATION_UNSUPPORTED)
                details.append(f"Multiple unverified claim indicators ({indicator_count})")
    ...
    def _find_fabricated_sources(
        ...
        for source in claimed:
            ...
            if not any(
                source_clean in actual_src.lower() or 
                actual_src.lower() in source_clean
    ...
    def _check_unsupported_claims(
        ...
        num_pattern = re.compile(r'\b(\d{2,})\s+(?:people|cases|deaths|patients)', re.IGNORECASE)
