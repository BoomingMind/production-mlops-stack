...
class GuardrailLogger:
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        total_events = sum(
            self._stats.get(et.value, 0) 
            for et in GuardrailEventType
        )
        
        blocked = (
            self._stats.get("input_blocked", 0) + 
            self._stats.get("output_blocked", 0)
        )
        ...
        return {
            ...
            "events_by_type": {
                et.value: self._stats.get(et.value, 0) 
                for et in GuardrailEventType
            },
