...
class TestPromptTemplates:
    ...
    PROMPT_DIR = Path(__file__).parent.parent / "experiments" / "prompts" / "prompt_templates"
    REQUIRED_TEMPLATES = ["zero_shot.txt", "few_shot_k3.txt", "few_shot_k5.txt", "chain_of_thought.txt", "meta_prompt.txt"]
    ...
    def test_prompt_templates_have_placeholders(self):
        ...
        for template_name in self.REQUIRED_TEMPLATES:
            ...
            assert "{aqi_value}" in content or "aqi" in content.lower(), \
    ...
    def test_prompt_templates_request_structured_output(self):
        ...
        for template_name in self.REQUIRED_TEMPLATES:
            ...
            has_json_example = "{{" in content or '{"' in content or "response:" in content
            assert has_json_keyword or has_json_example, \
...
class TestEvalDataset:
    ...
    def test_eval_dataset_valid_jsonl(self):
        ...
        with open(self.EVAL_DATA_PATH) as f:
            ...
            assert line_count >= 5, f"Eval dataset should have at least 5 examples, got {line_count}"
...
class TestPromptEvaluatorModule:
    ...
    def test_evaluator_module_exists(self):
        ...
        eval_script = Path(__file__).parent.parent / "experiments" / "prompts" / "evaluate_prompts.py"
    ...
    def test_evaluator_has_eval_class(self):
        ...
        assert "class PromptEvaluator" in content, \
